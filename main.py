...
class DialogueBox:
    ...
    def __init__(self, height=75, padding=10, border_thickness=2, textSpeed=50):
    ...
    def drawBackground(self, img):
        ...
        bg_end_point = (self.endX - self.border_thickness,
                        self.endY - self.border_thickness)
        bg_color = (0, 0, 0)
        bg_thickness = -1
    ...
    def drawBorders(self, backgroundImg):
        ...
        box = cv2.rectangle(backgroundImg, border_start_point,
                            border_end_point, border_color, self.border_thickness)

        return box

    def updateText(self):
        now = time.time()
        if (now - self.lastUpdate < 1 / self.textSpeed):
            return
    ...
    while True:
        ...
        with mic as source:
            ...
            try:
                ...
                dialogueBox.setText(''.join(value))
            except:
                print("COULDN'T RECOGNIZE SPEECH")


speechThread = threading.Thread(target=detectSpeech)
speechThread.start()
while True:
    ret, frame = cap.read()
